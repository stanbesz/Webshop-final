...
<body class="bg-image">
    <div class="container">
        ...
        if (empty($selector) || empty($validator)) {
            echo "Could not validate reset request";
        } else {
        ...
        ?>
                ...
                <div class="container position-absolute top-50 start-50 p-5 translate-middle text-center bg-dark text-light min-vh-20 col-4" style="border-radius: 25px;">
                    ...
                    <h1 class="h4 my-2 pt-2 font-weight-normal text-center border-top"> Enter new password:</h1>
                    <form style="max-width:600" action="include/reset_password_inc.php" method="post" >
                        <input type="hidden" name="selector" value="<?= $selector ?>">
                        <input type="hidden" name="validator" value="<?= $validator ?>">
                        <input type="password" id="password_reset" name="password_reset" class="form-control my-2" placeholder="Enter new password" required>
                        <input type="password" id="confirmPassword_reset" name="confirmPassword_reset" class="form-control my-2" placeholder="Confirm password" required>
                        <hr />
                        <button type="submit" name="reset_password_submit" class="btn btn-lg btn-primary btn-block mt-2 col-12">Reset password</button>
                    </form>
                    <p></p>
                    <small class="mt-3" style="font-size: 15px; color:#C3C2C1">@Stanimir Aleksandrov</small>
                </div>
        <?php
            }
        }
